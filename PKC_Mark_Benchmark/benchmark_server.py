...
def get_system_info():
    ...
    info = {
        ...
        "llama_cpp_info": captured_llama_info if captured_llama_info else {"status": "Available after model load"}
    }
    
    if NVIDIA_SMI_AVAILABLE:
        nvml_needs_cleanup = False
        try:
            try:
                nvmlDeviceGetCount()
            except NVMLError as e:
...
class BenchmarkRunner:
    ...
    def cleanup_nvml(self):
        ...
        if self.nvml_initialized and NVIDIA_SMI_AVAILABLE:
            try:
                ...
                print(f"⚠️ NVML Shutdown Failed: {e}")
    ...
    async def run_sequential_tests(self, original_prompt: str):
        """Sequential loading: load-test-unload each model individually."""
        ...
        # [REMOVED] Language-specific system instruction
        system_instruction = "You are a helpful AI assistant."

        effective_prompt_for_llms = f"<<SYS>>\n{system_instruction}\n<</SYS>>\n\n[Request] {original_prompt}\n" 
        
        connect_pipeline = getattr(self.params, 'connect_pipeline', True)
        await send_log("log", f"🔗 Connection Pipeline: {'Enabled' if connect_pipeline else 'Disabled'}")
        
        # 1. Analysis Models
        if analysis_models:
            await send_log("log", f"  -> 🧠 Stage 1: Running Analysis Models sequentially ({len(analysis_models)})")
            
            for model_id in analysis_models:
                if benchmark_status["cancelled"]: return
                
                await send_log("log", f"    📥 Loading {MODELS.get(model_id, {}).get('name', model_id)}...")
                success, result_data = await self._test_wrapper_sequential(model_id, original_prompt, original_prompt)
        ...
        effective_prompt_for_llms += "[Response]"
        
        # 2. Language Models
        if language_models:
            log_msg = f"  -> 🤖 Stage 2: Running Language Models sequentially ({'Analysis Injected' if analysis_models and connect_pipeline else 'Independent Run'})"
            await send_log("log", log_msg)
            for model_id in language_models:
                if benchmark_status["cancelled"]: break
                await send_log("log", f"    📥 Loading {MODELS.get(model_id, {}).get('name', model_id)}...")
                llm_prompt_to_use = effective_prompt_for_llms if connect_pipeline else (f"<<SYS>>\n{system_instruction}\n<</SYS>>\n\n[Request] {original_prompt}\n[Response]")
                await self._test_wrapper_sequential(model_id, llm_prompt_to_use, original_prompt)
                await send_log("log", f"    📤 Unloaded {MODELS.get(model_id, {}).get('name', model_id)}")

        # 3. Image Models
        if image_models:
            await send_log("log", "  -> 🖼️ Stage 3: Running Image Models sequentially")
            for model_id in image_models:
                if benchmark_status["cancelled"]: return
                await send_log("log", f"    📥 Loading {MODELS.get(model_id, {}).get('name', model_id)}...")
                await self._test_wrapper_sequential(model_id, original_prompt, original_prompt)
                await send_log("log", f"    📤 Unloaded {MODELS.get(model_id, {}).get('name', model_id)}")
                
        # 4. Other Models
        if other_models:
            await send_log("log", "  -> 🧩 Stage 4: Running Other Models sequentially")
    ...
    async def _test_wrapper_sequential(self, model_id: str, prompt: str, original_prompt: str) -> Tuple[bool, Optional[Dict]]:
        """Test wrapper for sequential mode: includes forced unload."""
        result = {"status": "❌ Failed", "prompt": original_prompt}
        model = None
        
        try:
            model_info = MODELS.get(model_id)
            if not model_info: raise ValueError(f"No definition for {model_id} in models config")
    ...
    async def run_parallel_tests(self, original_prompt: str):
        ...
        # 1. Analysis Models
        if analysis_models:
            await send_log("log", f"  -> 🧠 Stage 1: Analyzing context with {len(analysis_models)} models")
            for model_id in analysis_models:
                if benchmark_status["cancelled"]: return
        ...
        # 2. Language Models
        if language_models:
            log_msg = f"  -> 🤖 Stage 2: Testing Language Models ({'Analysis Injected' if analysis_models and connect_pipeline else 'Independent Run'})"
            llm_prompt_to_use = effective_prompt_for_llms if connect_pipeline else (f"<<SYS>>\n{system_instruction}\n<</SYS>>\n\n[Request] {original_prompt}\n[Response]")
            await send_log("log", log_msg)
            for model_id in language_models:
                if benchmark_status["cancelled"]: return
                await self._test_wrapper_parallel(model_id, llm_prompt_to_use, original_prompt)

        # 3. Image Models
        if image_models:
            await send_log("log", "  -> 🖼️ Stage 3: Testing Image Models")
            for model_id in image_models:
                if benchmark_status["cancelled"]: return
                await self._test_wrapper_parallel(model_id, original_prompt, original_prompt)

        # 4. Other Models
        if other_models:
            await send_log("log", "  -> 🧩 Stage 4: Testing Other Models")
            for model_id in other_models:
                if benchmark_status["cancelled"]: return
                await self._test_wrapper_parallel(model_id, original_prompt, original_prompt)
...
async def clear_cache_endpoint():
    global captured_llama_info; captured_llama_info = {}; model_cache.clear(); gc.collect()
