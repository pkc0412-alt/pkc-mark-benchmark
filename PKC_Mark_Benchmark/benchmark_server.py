...
def scan_models_directory(scan_path: str) -> Dict[str, Any]:
    ...
    for root, dirs, files in os.walk(scan_path, topdown=True, followlinks=True):
        ...
        if "config.json" in files:
            category = "language_model" # Default
            
            # --- Try to load config to determine category ---
            try:
                config_path = os.path.join(root, "config.json")
                # Use AutoConfig to check architecture
                config = AutoConfig.from_pretrained(config_path) 
                
                if config.architectures:
                    if "ForSequenceClassification" in config.architectures[0]:
                        category = "analysis_model"
                    elif "ForQuestionAnswering" in config.architectures[0]:
                        # Treat Q&A as 'analysis' for pipeline purposes
                        category = "analysis_model" 
                    elif "ForCausalLM" in config.architectures[0]:
                        category = "language_model"
                    # Add other specific architectures if needed
            except Exception:
                # Fallback to old folder name logic if config loading fails
...
class BenchmarkRunner:
    ...
    async def _run_tests(self, original_prompt: str, strategy: str = 'parallel'):
        ...
        if analysis_models:
            ...
            for model_id in analysis_models:
                ...
                if success and result_data:
                    ...
                    # [MODIFIED] Determine analysis_type based on Q&A or Classification
                    is_qa_model = (MODELS.get(model_id, {}).get('type') == 'transformers') and ('Answer:' in analysis_result)
                    
                    if is_qa_model:
                        analysis_type = "Context" # Q&A results are injected as Context
                        analysis_result = analysis_result.replace('Answer:', '').strip()
                    else:
        ...
        if language_models:
            llm_prompt_to_use = effective_prompt_for_llms if (analysis_models and connect_pipeline) else f"<<SYS>>\n{system_instruction}\n<</SYS>>\n\n[Request] {original_prompt}\n[Response]"
            await send_log("log", f"  -> 🤖 Stage 2: Testing Language Models ({len(language_models)}) — Pipeline: {'Connected' if (analysis_models and connect_pipeline) else 'Independent'}")
            for model_id in language_models:
                if benchmark_status["cancelled"]: return
                success, _ = await self._test_wrapper_parallel(model_id, llm_prompt_to_use, original_prompt)
                if strategy == 'sequential':
                    try:
                        await self.unload_model(model_id)
                        await send_log("log", f"    📤 Unloaded {MODELS.get(model_id, {}).get('name', model_id)}")
        ...
        # Stage 3: Image Models
        if image_models:
            await send_log("log", f"  -> 🖼️ Stage 3: Testing Image Models ({len(image_models)})")
        ...
        if other_models:
            await send_log("log", f"  -> 🧰 Stage 4: Testing Other Models ({len(other_models)})")
    ...
    def _run_transformers_inference(self, model_tuple, prompt, model_info):
        ...
        if task_type == "classification":
            ...
            # [MODIFIED] Use a tag to split Question/Context
            parts = prompt.split("[Context]", 1)
            question = parts[0].replace("[Question]", "").strip()
            context = parts[1].strip() if len(parts) > 1 else ""

            if not question or not context:
                # If no tag, try to split by 'Context:' as a fallback
                fallback_parts = prompt.split("Context:", 1)
                question = fallback_parts[0].replace("Question:", "").strip()
                context = fallback_parts[1].strip() if len(fallback_parts) > 1 else ""
                
                if not question or not context:
                    output_text = f"⚠️ Q&A Format Error: Expected '[Question] [Q] [Context] [C]'"
                else:
                    output_text = self._perform_qa(tokenizer, model_instance, question, context)
            else:
                output_text = self._perform_qa(tokenizer, model_instance, question, context)
    ...
    def _perform_qa(self, tokenizer, model_instance, question, context):
        """Helper for Q&A inference."""
