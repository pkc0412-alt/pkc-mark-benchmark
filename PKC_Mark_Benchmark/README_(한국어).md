PKC MARK 벤치마크 툴 Beta V0.9

⚠️ 중요 공지 (Important Notice)
이 벤치마크 툴의 사용자 인터페이스(UI) 및 내부 로그, 태그는 모두 영어(English)로 통일되었습니다.
이 한글 문서는 영문 UI를 기준으로 기능을 설명합니다.
...
파이프라인 테스트:
분석 모델(예: 'Emotion' 또는 'Analysis' 태그가 붙은 모델)의 결과를 언어 모델(LLM)의
프롬프트에 자동으로 주입하여 연계 테스트를 수행할 수 있습니다.
...
서버 실행: start_server_windows.bat 파일을 더블 클릭하여 실행합니다.
(또는 터미널에서 python benchmark_server.py 입력)

UI 접속: 서버가 시작되면 잠시 후 자동으로 기본 웹 브라우저에서 benchmark_canvas.html 페이지가 열립니다.
(안내: UI의 모든 텍스트는 영어로 표시됩니다.)

설정: 테스트할 모델, 프롬프트, GPU 레이어 수, 캐싱 여부 등을 웹 UI에서 설정합니다.

실행: '🚀 Start Benchmark' 버튼을 클릭하여 테스트를 시작합니다.
